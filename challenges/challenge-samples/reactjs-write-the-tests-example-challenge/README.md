The React counter in `App.jsx` is already written and **correct**. Your task is the reverse of a usual challenge: write the **tests** in `App.spec.js` (Jest and `@testing-library/react`) that prove it works, the way the starter test does.
