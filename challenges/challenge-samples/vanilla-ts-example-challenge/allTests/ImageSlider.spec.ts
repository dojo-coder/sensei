...
describe("ImageSlider", () => {
  ...
  test("disables previous button when clicking previous button and the first image is displayed", () => {
    imageSlider(images);
    const prevBtn = document.querySelector(
      "button:first-of-type"
    ) as HTMLButtonElement;
  ...
  test("disables the next button when reaching the last image in images list", () => {
    imageSlider(images);
    const nextBtn = document.querySelector(
      "button:last-of-type"
    ) as HTMLButtonElement;
