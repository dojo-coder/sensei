The vanilla TypeScript todo list in `todoList.ts` is already written and **correct**. Your task is the reverse of a usual challenge: write the **tests** in `todoList.spec.ts` (Jest and jsdom) that prove it works, the way the starter test does.
...
- The vanilla template has no Testing Library: reset the page from `globalThis.htmlContent` (the contents of `index.html`) in a `beforeEach`, call `createTodoList(document.getElementById("app") as HTMLElement)`, and query the page with plain DOM calls such as `document.querySelector("input")`, as the starter test does.
