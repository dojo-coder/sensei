The React + TypeScript todo list in `App.tsx` is already written and **correct**. Your task is the reverse of a usual challenge: write the **tests** in `App.spec.ts` (Jest and `@testing-library/react`) that prove it works, the way the starter test does.
