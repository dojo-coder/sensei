...
class Ship:
    ...
    def steer(self, keys):
        # TODO: move the ship by SPEED pixels per frame.
        # - Left arrow or A moves left, right arrow or D moves right
        #   (read them as keys[pygame.K_LEFT], keys[pygame.K_a], ...).
        # - The ship must stay fully on screen: its centre never goes below
        #   SIZE / 2 or above WIDTH - SIZE / 2.
        pass

    def hull(self):
        # TODO: return the ship's hit box as a pygame.Rect.
        # It is the SIZE x SIZE square centred on (x, y), shrunk by 6 pixels
        # on every side so that near misses stay near misses.
...
class Asteroid:
    ...
    def update(self):
        # TODO: advance the asteroid by one frame.
        # - It falls by `speed` and slides sideways by `drift`.
        # - After moving, if it sticks out past the left or right edge
        #   (x < radius or x > WIDTH - radius), it bounces: `drift` flips sign.
        pass

    def bounds(self):
        # TODO: return the square pygame.Rect that wraps the asteroid:
        # centred on (x, y), with a side of twice the radius.
        return pygame.Rect(0, 0, 0, 0)
